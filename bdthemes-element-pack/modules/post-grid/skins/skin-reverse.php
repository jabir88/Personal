					...
					if($bdt_count == 3) {
		  			...
		  			if ( $bdt_count % 2 != 0) {
						...
						<div class="<?php echo esc_attr($bdt_grid_raw . $bdt_post_class); ?>">
							<?php $this->render_post_grid_layout_plane( $post->ID, $settings['thumbnail_size'], $settings['excerpt_length']); ?>
						</div>
						<?php
		  			} else {
						$bdt_grid_raw   = ' bdt-width-1-' . esc_attr($settings['columns']) . '@m bdt-width-1-' . esc_attr($settings['columns_tablet']) . '@s bdt-width-1-' . esc_attr($settings['columns_mobile']) ;
						...
						?>
						<div class="<?php echo esc_attr($bdt_grid_raw . $bdt_post_class); ?>">
							<?php $this->render_post_grid_layout_reverse( $post->ID, $settings['thumbnail_size'], $settings['excerpt_length']); ?>
						</div>
						<?php
		  			}
