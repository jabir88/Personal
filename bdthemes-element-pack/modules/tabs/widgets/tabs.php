...
class Tabs extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			'item_spacing',
			[
				'label' => __( 'Tab Spacing', 'bdthemes-element-pack' ),
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-tab-wrapper' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .bdt-tabs-left .bdt-switcher-wrapper'   => 'margin-left: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .bdt-tabs-right .bdt-switcher-wrapper'  => 'margin-right: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	public function desktop_tab_items() {
		...
		if ( 'left' == $settings['tab_layout'] or 'right' == $settings['tab_layout'] ) {
			...
			if ( 960 == $settings['media'] ) {
				$this->add_render_attribute( 'tabs-width',  'class', 'bdt-width-auto@m' );
			} elseif (768 == $settings['media']) {
				$this->add_render_attribute( 'tabs-width',  'class', 'bdt-width-auto' );
				if ( 'right' == $settings['tab_layout'] ) {
				...
				'tab-settings' => [
					...
					'bdt-tab' => [
						wp_json_encode(array_filter([
							"connect"   => "#bdt-tab-content-" .  esc_attr($id),
