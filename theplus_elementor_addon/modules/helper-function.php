...
function theplus_get_contact_form_post() {
	...
	$cf7 = get_posts('post_type="wpcf7_contact_form"&numberposts=-1');
		if ($cf7) {
			...
			$contact_forms[0] = esc_html__('No contact forms found', 'theplus');
		}
...
function theplus_get_everest_form_post() {
	...
	$ev_form = get_posts('post_type="everest_form"&numberposts=-1');
		if ($ev_form) {
			...
			$everest_form[0] = esc_html__('No everest forms found', 'theplus');
		}
	return $everest_form;
}
/*-everest form-*/
...
function theplus_gravity_form() {
    if ( class_exists( 'GFCommon' ) ) {
     $gravity_forms = \RGFormsModel::get_forms( null, 'title' );
        ...
        $g_form_options = ['0' => esc_html__( 'Form Not Found!', 'theplus' ) ];
		}
...
function theplus_pc_form( $settings,$widget_id ) {
    echo '<div class="theplus-password-pc-fields">';
		...
		if(isset($_POST['protection_password'.$widget_id]) && !isset($_SESSION['protection_password'.$widget_id])) {
			echo '<p class="theplus-pc-error-msg">'.esc_html($error_msg).'</p>';
		}
    echo '</div>';
}
...
function theplus_breadcrumbs($icontype='',$sep_icontype='',$icons='',$home_titles='',$sep_icons='',$active_page_text_default='',$breadcrumbs_last_sec_tri_normal='',$breadcrumbs_on_off_home='',$breadcrumbs_on_off_parent='',$breadcrumbs_on_off_current='',$letter_limit_parent='',$letter_limit_current='') {
		...
		if($icons_content != '' || $icons_sep_content != '' ||  $text['home'] != ''){
			if($breadcrumbs_on_off_home != ''){
				$home_link = '<span class="bc_home"><a class="home_bread_tab on-off-home" href="%1$s">'.$icons_content.'%2$s'.$icons_sep_content.'</a>' . $linkAfter;
			}else {
	   ...
	   if($breadcrumbs_on_off_parent != ''){
		   $link = '<span class="bc_parent"><a class="parent_sub_bread_tab on-off-parent" href="%1$s">%2$s'.$icons_sep_content.'</a>' . $linkAfter;
	   }else {   
