			...
			if(!empty($settings['content_loop_section'])){
				foreach($settings['content_loop_section'] as $item){
					...
					if(!empty($item['loop_pin_select_icon'])){
						if(isset($item['loop_pin_select_icon']) && $item['loop_pin_select_icon'] == 'image'){
							...
							if(!empty($item["loop_pin_image"]["url"])){								
								...
								$img = wp_get_attachment_image_src($loop_pin_image,$item['loop_pin_image_thumbnail_size']);
								$loop_imgSrc = $img[0];
					...
					if(!empty($item['loop_featured_image']['url']) && $item["loop_content_options"]=='image'){
						...
						$loop_featured_image=$item['loop_featured_image']['id'];
						$img = wp_get_attachment_image_src($loop_featured_image,$item['loop_featured_image_thumbnail_size']);
						$loop_featured_image_Src = $img[0];
						
						$content_image='<img src="'.esc_url($loop_featured_image_Src).'" class="hover__img" alt="'.esc_attr($image_alt).'" />';						
					}
		...
		if(!empty($settings['start_pin_select_icon'])){
			if(isset($settings['start_pin_select_icon']) && $settings['start_pin_select_icon'] == 'image'){
				...
				if(!empty($settings["start_pin_image"]["url"])){
					...
					$start_pin_image=$settings['start_pin_image']['id'];
					$img = wp_get_attachment_image_src($start_pin_image,$settings['start_pin_image_thumbnail_size']);
					$loop_imgSrc = $img[0];
				}else{
					$loop_imgSrc='';
				}
		...
		if(!empty($settings['end_pin_select_icon'])){
			if(isset($settings['end_pin_select_icon']) && $settings['end_pin_select_icon'] == 'image'){
				...
				if(!empty($settings["end_pin_image"]["url"])){
					...
					$end_pin_image=$settings['end_pin_image']['id'];
					$img = wp_get_attachment_image_src($end_pin_image,$settings['end_pin_image_thumbnail_size']);
					$loop_imgSrc = $img[0];
				}else{
					$loop_imgSrc='';
				}
