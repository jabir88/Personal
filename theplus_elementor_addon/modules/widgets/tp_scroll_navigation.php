...
class ThePlus_Scroll_Navigation extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				'label' => esc_html__( 'Navigation Dispaly Counter Margin', 'theplus' ),
				'type' => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', 'em', '%' ],				
				'selectors' => [
	 ...
	 protected function render() {
		...
		if ( $settings['scroll_navigation_menu_list'] ) {
			$uid=uniqid('scroll');
			$scroll_navigation = '<div '.$id.' class="theplus-scroll-navigation '.esc_attr($uid).' '.esc_attr($scroll_style).' '.esc_attr($direction_class).' '.esc_attr($position_class).' '.esc_attr($show_scroll_window_offset).'" '.$scroll_top_offset_value.' data-uid="'.esc_attr($uid).'" '.$full_page_connect.'>';			
