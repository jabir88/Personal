...
class ThePlus_Dynamic_Smart_Showcase extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .bss-list .bss-wrapper .bss-wrap:hover .entry-content,{{WRAPPER}} .bss-list .bss-wrapper .bss-wrap:hover .entry-content p' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_tab();
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .bss-list .post-inner-loop .bss-wrap:hover .post-meta-info span,{{WRAPPER}} .bss-list .post-inner-loop .bss-wrap:hover .post-meta-info span a,{{WRAPPER}} .bss-list .post-inner-loop .bss-wrap:hover .post-meta-info i,
	...
	protected function render_title() {
		$settings = $this->get_settings_for_display();
		$post_title_count=$settings["post_title_count"];
	...
	public function render_date() {
		...
		return '<span class="theplus-post-date "><a href="'.esc_url(get_the_permalink()).'">'.esc_attr(get_the_date('M d, Y')).'</a></span>';		
	}
	 ...
	 protected function render() {
		...
		if ( ! $query->have_posts() ) {
			...
			if( $settings['show_tricker'] == 'yes' && ($style == 'magazine' || $style == 'none')){			
				...
				$output .='<div class="theplus-news-ticker '.$animated_class.' '.$ai_res_tab_class.' '.$ai_res_mob_class.' '.$date_res_tab_class.' '.$date_res_mob_class.' '.$an_res_tab_class.' '.$an_res_mob_class.'" '.$animation_attr.'>';
					...
					if(!empty($style)){
						...
						if($ji==1){
							$bssfc='bssfc';
								...
								echo '<div class="grid-item '.$active.' '.$category_filter.' active '.$bssfc.' '.$news_highlight_style.'">';					
