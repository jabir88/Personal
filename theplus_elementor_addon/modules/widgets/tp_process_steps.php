    ...
    protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .tp-process-steps-widget .tp-process-steps-wrapper .tp-ps-content .tp-pro-step-desc' => 'color: {{VALUE}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					{{WRAPPER}} .tp-process-steps-widget .tp-process-steps-wrapper.active .tp-ps-content .tp-pro-step-desc' => 'color: {{VALUE}};',
				],
			]
		);
		$this->end_controls_section();
				...
				$output = '<div id="'.$uid.'" class="tp-process-steps-widget '.$settings['ps_style'].' '.$seprator_cusom_img_class.' '.$mobile_class.' '.$settings['img_st2_align'].' '.$settings['content_st2_align'].' '.$animated_class.'" '.$animation_attr.' data-connection="'.esc_attr($connect_carousel).'" data-eventtype="'.esc_attr($connection_hover_click).'">';	
					...
					foreach($loop_content as $index => $item) {
						...
						if ( ! empty( $item['loop_url_link']['url'] ) ) {
							$this->add_render_attribute( 'loop_box_link', 'href', $item['loop_url_link']['url'] );
						...
						if(!empty($item['loop_title'])){							
							if (!empty($item['loop_url_link']['url'])){
								$title_a_start = '<a '.$this->get_render_attribute_string( "loop_box_link" ).'>';
								$title_a_end = '</a>';
							}							
