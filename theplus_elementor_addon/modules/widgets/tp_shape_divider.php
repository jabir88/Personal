    ...
    protected function _register_controls() {
		...
		$repeater->add_control(
			'shape_color',[
				...
				'default' => '#8072fc',
				'selectors' => [
					'.tp-plus-shape-divider .wave-items{{CURRENT_ITEM}} path' => 'fill: {{VALUE}}'
				],
			]
		);
		$repeater->add_control(
	 ...
	 protected function render() {
		...
		$divider_position = ($settings["divider_position"]!='') ? $settings["divider_position"] : 'bottom';
		if(!empty($settings["shape_divider_type"]) && $settings["shape_divider_type"]=='column'){
			$divider_position = ($settings["divider_position_column"]!='') ? $settings["divider_position_column"] : 'right';
		}
		...
		$output ='<div id="'.esc_attr($uid).'" class="tp-plus-shape-divider '.esc_attr($data_class).' " '.$data_attr.'>';
			if($shape_divider=='wave' && !empty($settings['wave_loop_shape'])){
				...
				foreach($settings['wave_loop_shape'] as $item) {
					$shape_color =($item['shape_color']!='') ? 'data-color="'.$item['shape_color'].'"' : '#8072fc';
					$shape_height =($item['shape_height']["size"]!='') ? 'data-height="'.$item['shape_height']["size"].'"' : '80';
					$shape_bones =($item['shape_bones']["size"]!='') ? 'data-bones="'.$item['shape_bones']["size"].'"' : '4';
					$shape_amplitude =($item['shape_amplitude']["size"]!='') ? 'data-amplitude="'.$item['shape_amplitude']["size"].'"' : '60';
					$shape_speed =($item['shape_speed']["size"]!='') ? 'data-speed="'.$item['shape_speed']["size"].'"' : 0.15;
					
					$output .='<svg class="wave-items elementor-repeater-item-' . esc_attr($item['_id']) . '" width="100%" height="'.esc_attr($item['shape_height']["size"]).'" version="1.1" xmlns="http://www.w3.org/2000/svg" class="wave" '.$shape_color.' '.$shape_height.' '.$shape_bones.' '.$shape_amplitude.' '.$shape_speed.'><defs></defs><path id="wave-' . esc_attr($item['_id']) . '" d="" /></svg>';
				}
			}
