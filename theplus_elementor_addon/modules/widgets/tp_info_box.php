    ...
    protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'no',
				'condition' => [
					'main_style' => ['style_1','style_2','style_3','style_4','style_5','style_6'],
				],
			]
		);
		$this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .pt_plus_info_box .info-box-inner:hover .service-icon:before,{{WRAPPER}} .pt_plus_info_box .info-box-inner.tp-info-active .service-icon:before' => 'color: {{VALUE}};background: transparent;-webkit-background-clip: unset;-webkit-text-fill-color: initial;',
				],
				'condition' => [
		...
		$this->add_control(
			'pin_width',
			[
				'label' => esc_html__( 'Pin Size', 'theplus' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px' ],
				'range' => [
					'px' => [
				...
				'selectors' => [
					'{{WRAPPER}} .pt_plus_info_box .info-box-inner .info-pin-text.square-pin' => 'width: {{SIZE}}{{UNIT}};height: {{SIZE}}{{UNIT}};line-height: {{SIZE}}{{UNIT}} !important;',
				],
				'condition' => [
					'square_pin' => 'yes',
				],
	 ...
	 protected function render() {
		...
		$full_infobox_link = $settings["full_infobox_link"]["url"];
		$fc_load_class='';
		//r_full_infobox_switch
		...
		if(!empty($settings['tilt_parallax']) && $settings['tilt_parallax']=='yes'){
			...
			$this->add_render_attribute( 'tilt_parallax', 'data-tilt-max', $tilt_max , true );
			$this->add_render_attribute( 'tilt_parallax', 'data-tilt-perspective', $tilt_perspective , true );
			$this->add_render_attribute( 'tilt_parallax', 'data-tilt-speed', $tilt_speed , true );
			
			if($settings["plus_tilt_opt_tilt_easing"] !='custom'){
				$easing_tilt=$settings["tilt_opt_tilt_easing"];					
			}else if($settings["plus_tilt_opt_tilt_easing"] =='custom'){
				$easing_tilt=$settings["tilt_opt_tilt_easing_custom"];
			}else{
				$easing_tilt='cubic-bezier(.03,.98,.52,.99)';
			}
			$this->add_render_attribute( 'tilt_parallax', 'data-tilt-easing', $easing_tilt , true );
				...
				foreach($settings["loop_content"] as $item) {
						...
						if(!empty($item['loop_title'])){
							...
							if ((!empty($item['loop_url_link']['url'])) && ($item['r_full_infobox_switch'] !='yes')){
								$list_title = '<a '.$this->get_render_attribute_string( "loop_box_link".$index ).'><h6 class="service-title ">'.esc_html($loop_title).'</h6></a>';						
							}else{
							...
							$loop_max_width= $item['loop_max_width']["size"].$item['loop_max_width']["unit"];
								...
								if(isset($item['loop_image_icon']) && $item['loop_image_icon'] == 'image'){
									...
									$list_img ='<div class="ts-icon-img icon-img-b " >';
										$list_img .='<img class="" src='.esc_url($loop_imgSrc).' alt="'.esc_attr($image_alt).'" />';
									$list_img .='</div>';
		...
		if ($info_box_layout == 'single_layout'){
			...
			if($main_style == 'style_3'){
				...
				$output .= '<div class="info-box-bg-box '.esc_attr($icon_overlay_style).' '.esc_attr($serice_box_border).'  '.esc_attr($inner_js_tilt).'" '.$this->get_render_attribute_string( 'tilt_parallax' ).'>';
					$output .= '<div class="'.esc_attr($service_align).'">';
						$output .= '<div class="service-center  ">';
