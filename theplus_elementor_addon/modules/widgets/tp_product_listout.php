    ...
    protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'separator' => 'before',
				'condition' => [
					'product_post_listing!' => ['archive_listing','related_product'],
				],
			]
		);
		$this->add_control(
		...
		$this->add_control(
			'include_products',
			[
				'label'       => esc_html__( 'Include Product(s)', 'theplus' ),
		...
		$this->add_control(
			'display_cart_button',
			[
				'label' => esc_html__( 'Cart Button Display', 'theplus' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .product-list .wrapper-cart-price .price .amount',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .product-list .wrapper-cart-price .price .amount' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_tab();
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .product-list .product-list-content:hover .wrapper-cart-price .price .amount' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_tab();
		...
		$this->add_group_control(
			...
			[
				...
				'label' => esc_html__( 'Price Typography', 'theplus' ),
				'scheme'   => Scheme_Typography::TYPOGRAPHY_3,
				'selector' => '{{WRAPPER}} .product-list .wrapper-cart-price .price del .amount',
			]
		);
		...
		$this->add_control(
			'sale_price_color',
			[
				'label' => esc_html__( 'Sale Price Color', 'theplus' ),
				'type' => Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .product-list .wrapper-cart-price .price del .amount' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_tab();
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .product-list .product-list-content:hover .wrapper-cart-price .price del .amount' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_tab();
		...
		$this->start_controls_section(
            'section_badge_style',
            [
                'label' => esc_html__('Badge Style', 'theplus'),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        );
		$this->add_control(
	 ...
	 protected function render() {
		...
		if ( !class_exists('woocommerce') ) {
			...
			$output .='<h3 class="theplus-posts-not-found">'.esc_html__( "Products not found", "theplus" ).'</h3>';
		}else{
			$output .= '<div id="pt-plus-product-list" class="product-list '.esc_attr($uid).' '.esc_attr($data_class).' '.$animated_class.'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
			...
			while ( $query->have_posts() ) {
				...
				if(!empty($style)){
					ob_start();
					include THEPLUS_PATH. 'includes/product/product-'.esc_attr($style).'.php'; 
