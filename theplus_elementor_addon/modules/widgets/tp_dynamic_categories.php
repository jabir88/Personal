...
class ThePlus_Dynamic_Categories extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_control(
			'post_category',
			[
				'label'       => esc_html__( 'Terms ID', 'theplus' ),
	 ...
	 protected function render() {
		...
		if (!$dynamic_categories) {
			...
			$output .= '<div id="pt-plus-dynamic-cat-list" class="dynamic-cat-list '.esc_attr($uid).' '.esc_attr($data_class).' '.$animated_class.'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
