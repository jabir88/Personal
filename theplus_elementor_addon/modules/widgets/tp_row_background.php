...
class ThePlus_Row_Background extends Widget_Base {
	...
	protected function _register_controls() {
	 ...
	 protected function render() {
		...
		$columns_parallax_sense = $settings['columns_parallax_sense']['size'];
		...
		/*----------bg video---*/
			...
			if(!empty($settings['columns_video_variant']) && $select_anim=='bg_video') {
				...
				if(!empty($settings['responsive_video_mp4']) && $settings['responsive_video_mp4']=='yes' && !empty($settings['responsive_video_mp4_tablet'])){
					$mp4_video_tablet =$settings['responsive_video_mp4_tablet'];
				...
				if(!empty($settings['responsive_video_mp4']) && $settings['responsive_video_mp4']=='yes' && !empty($settings['responsive_video_mp4_mobile'])){
					$mp4_video_mobile =$settings['responsive_video_mp4_mobile'];
				...
				if($settings['columns_video_variant']== 'self-hosted' && (!empty($settings['columns_video_url_mp4']) || !empty($settings['columns_video_url_webm']))){
					...
					$output .= '<div class="pt-plus-bg-video pt-plus-columns-bg-wrap columns-video-bg self-hosted-video-bg '.esc_attr($video_responsive_poster).'" id="wrapper-'.esc_attr($uniqid2).'" '.$data_atts2.' style="background-image: url('.esc_js($poster_url).');" '.esc_attr($responsive_video_image).'>';
						$output .= '<video id="'.esc_attr($uniqid2).'" class="self-hosted-videos video-js vjs-default-skin columns_vc_hidden-md columns_vc_hidden-sm columns_vc_hidden-xs"  autoplay preload="auto" width="100%" height="100%"  autoplay="true" playsinline '.$video_atts2.' data-setup="{}" '.$data_attr_video.'></video>';
					$output .= '</div>';
