...
class ThePlus_Dynamic_Listing extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Metro & Masonry Layouts needs extra care when you design them in custom skin.', 'theplus' ),
		...
		$this->add_control(
			...
			[
				...
				'condition' => [					
					'blogs_post_listing!' => ['archive_listing','related_product'],
					'query!' => ['post'],
					'post_taxonomies!' => '',
				],
			]
		...
		$this->add_group_control(
			...
			[
				...
				'label' => esc_html__( 'Typography', 'theplus' ),
				'scheme' => Scheme_Typography::TYPOGRAPHY_1,
				'selector' => '{{WRAPPER}} .dynamic-listing .post-inner-loop .entry-content',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .dynamic-listing.dynamic-listing-style-3 .blog-list-content,{{WRAPPER}} .dynamic-listing.dynamic-listing-style-3 .blog-featured-image' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'condition' => [
					'style' => 'style-3',
				],
		...
		$this->add_group_control(
			...
			[
				'name'     => 'image_shadow',
				'selector' => '{{WRAPPER}} .dynamic-listing.dynamic-listing-style-3 .blog-list-content .blog-featured-image',
				'condition' => [
					'style' => 'style-3',
				],
			]
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .dynamic-listing.dynamic-listing-style-3 .blog-list-content:hover .blog-featured-image',
				'condition' => [					
					'style' => 'style-3',
				],
			]
	...
	protected function render() {
		...
		if ( ! $query->have_posts() ) {
			...
			$output .= '<div id="pt-plus-dynamic-listing" class="dynamic-listing '.esc_attr($uid).' '.esc_attr($data_class).' '.esc_attr($style_layout).' '.$animated_class.'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
			...
			while ( $query->have_posts() ) {
				...
				if(!empty($style)){
					ob_start();
					include THEPLUS_PATH. 'includes/dynamic-listing/dl-'.esc_attr($style).'.php'; 
			...
			if($settings['post_extra_option']=='pagination' && $layout!='carousel'){
				...
				if(!empty($total_posts) && $total_posts>0){
					$output .= '<div class="ajax_load_more">';
						$output .= '<a class="post-load-more" data-load="dynamiclisting" data-post_type="'.$settings['query'].'" data-texonomy_category="'.$settings['post_taxonomies'].'" data-load-class="'.esc_attr($uid).'" data-layout="'.$layout.'" data-style="'.esc_attr($style).'" data-style_layout="'.esc_attr($style_layout).'" data-desktop-column="'.esc_attr($settings['desktop_column']).'" data-tablet-column="'.esc_attr($settings['tablet_column']).'" data-mobile-column="'.esc_attr($settings['mobile_column']).'" data-metro_column="'.esc_attr($settings['metro_column']).'" data-metro_style="'.esc_attr($metro_style).'" data-responsive_tablet_metro="'.esc_attr($responsive_tablet_metro).'" data-tablet_metro_column="'.esc_attr($tablet_metro_column).'" data-tablet_metro_style="'.esc_attr($tablet_metro_style).'"  data-offset-posts="'.($settings['post_offset']).'" data-category="'.esc_attr($dynamic_cat).'"  data-post_tags="'.esc_attr($post_tags).'" data-order_by="'.esc_attr($settings['post_order_by']).'" data-post_order="'.esc_attr($settings['post_order']).'" data-filter_category="'.esc_attr($settings['filter_category']).'" data-display_post="'.esc_attr($settings['display_posts']).'" data-animated_columns="'.esc_attr($animated_columns).'" data-post_load_more="'.esc_attr($settings['load_more_post']).'" data-page="1" data-total_page="'.esc_attr($load_page).'" data-display_post_meta="'.$display_post_meta.'" data-post_meta_tag_style="'.$post_meta_tag_style.'" data-display_excerpt="'.$display_excerpt.'" data-post_excerpt_count="'.$post_excerpt_count.'" data-display_post_category="'.$display_post_category.'" data-post_category_style="'.$post_category_style.'" data-featured_image_type="'.$featured_image_type.'" data-loaded_posts="'.esc_attr($loaded_posts_text).'" data-display_thumbnail="'.esc_attr($display_thumbnail).'" data-thumbnail="'.esc_attr($thumbnail).'" data-thumbnail_car="'.esc_attr($thumbnail_car).'" '.$button_attr.' data-skin_template=\''.json_encode($temp_array).'\'>'.esc_html($settings['load_more_btn_text']).'</a>';					
					$output .= '</div>';	
				}
			}else if($settings['post_extra_option']=='lazy_load' && $layout!='carousel'){
				if(!empty($total_posts) && $total_posts>0){	
					$output .= '<div class="ajax_lazy_load">';
						$output .= '<a class="post-lazy-load" data-load="dynamiclisting" data-post_type="'.$settings['query'].'" data-texonomy_category="'.$settings['post_taxonomies'].'" data-load-class="'.esc_attr($uid).'" data-layout="'.$layout.'" data-style="'.esc_attr($style).'" data-style_layout="'.esc_attr($style_layout).'" data-desktop-column="'.esc_attr($settings['desktop_column']).'" data-tablet-column="'.esc_attr($settings['tablet_column']).'" data-mobile-column="'.esc_attr($settings['mobile_column']).'" data-metro_column="'.esc_attr($settings['metro_column']).'" data-metro_style="'.esc_attr($metro_style).'" data-responsive_tablet_metro="'.esc_attr($responsive_tablet_metro).'" data-tablet_metro_column="'.esc_attr($tablet_metro_column).'" data-tablet_metro_style="'.esc_attr($tablet_metro_style).'"  data-offset-posts="'.($settings['post_offset']).'"  data-category="'.esc_attr($dynamic_cat).'"  data-post_tags="'.esc_attr($post_tags).'" data-order_by="'.esc_attr($settings['post_order_by']).'" data-post_order="'.esc_attr($settings['post_order']).'" data-filter_category="'.esc_attr($settings['filter_category']).'" data-display_post="'.esc_attr($settings['display_posts']).'" data-animated_columns="'.esc_attr($animated_columns).'" data-post_load_more="'.esc_attr($settings['load_more_post']).'" data-page="1" data-total_page="'.esc_attr($load_page).'"  data-display_post_meta="'.$display_post_meta.'" data-post_meta_tag_style="'.$post_meta_tag_style.'" data-display_excerpt="'.$display_excerpt.'" data-post_excerpt_count="'.$post_excerpt_count.'" data-display_post_category="'.$display_post_category.'" data-post_category_style="'.$post_category_style.'" data-featured_image_type="'.$featured_image_type.'" data-loaded_posts="'.esc_attr($loaded_posts_text).'" data-display_thumbnail="'.esc_attr($display_thumbnail).'" data-thumbnail="'.esc_attr($thumbnail).'" data-thumbnail_car="'.esc_attr($thumbnail_car).'" '.$button_attr.' data-skin_template=\''.json_encode($temp_array).'\'><img src="'.THEPLUS_ASSETS_URL. 'images/lazy_load.gif" /></a>';
						$output .= '</div>';
				}
			$output .='</div>';
			}
	...
	protected function get_query_args() {
		...
		}else if($offset && $settings['post_extra_option']=='pagination'){
			$page = max( 1, $paged );
