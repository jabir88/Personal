...
class ThePlus_Advanced_Typography extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$repeater->add_control(
			'marquee_scrolldelay',
			[
				'label' => esc_html__( 'Scroll Delay', 'theplus' ),
				'type' => Controls_Manager::NUMBER,
				'min' => 1,
				'max' => 1000,
				'step' => 5,
		...
		$this->add_control(
			...
			[	
				...
				'selectors'  => [
					'{{WRAPPER}} .pt-plus-adv-typo-wrapper .pt_plus_adv_typo_block .text-content-block' => 'animation-duration: {{SIZE}}{{UNIT}};-webkit-animation-duration: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'text_continuous_animation' => 'yes',
				],
	 ...
	 protected function render() {
		...
		if($typo_underline_style!='' && $typo_underline_style=='under_overlay'){
			$typo_underline_style .= ($settings['typo_overlay_under_style']!='') ? ' overlay-'.$settings['typo_overlay_under_style'] : '';
		}
		...
		if($settings['circular_text_switch']=='yes'){
			if($settings['circular_text_custom'] == 'yes'){
				$circular_radious = ($settings['circular_radious']['size']!='') ? $settings['circular_radious']['size'] : '360';
				$circular_attr .= ' data-custom-radius="' . esc_attr($circular_radious) . '" ';
			}
			if($settings['circular_text_reversed'] == 'yes'){				
				$circular_attr .= ' data-custom-reversed="yes" ';
			}
			if($settings['circular_text_resized'] == 'yes'){				
		...
		if($settings['background_based_text_switch']=='yes'){
				...
				if(!empty($settings["typography_listing"]) && $settings["typography_listing"]=='listing'){
					...
					if(!empty($settings["listing_content"])) {
						...
						foreach($settings["listing_content"] as $item) {
							...
							if($item['marquee_switch']=='yes'){
								$marquee_attr .= (!empty($item['marquee_direction'])) ? ' direction="'.$item['marquee_direction'].'"' : '';
								$marquee_attr .= (!empty($item['marquee_behavior'])) ? ' behavior="'.$item['marquee_behavior'].'"' : '';						
								$marquee_attr .= (!empty($item['marquee_loop'])) ? ' loop="'.$item['marquee_loop'].'"' : '';
								$marquee_attr .= (!empty($item['marquee_scrollamount'])) ? ' scrollamount="'.$item['marquee_scrollamount'].'"' : '';
								$marquee_attr .= (!empty($item['marquee_scrolldelay'])) ? ' scrolldelay="'.$item['marquee_scrolldelay'].'"' : '';
								$loop_tag='marquee';
							}
							...
							if(!empty($item['plus_mouse_move_parallax']) && $item['plus_mouse_move_parallax']=='yes'){
								...
								$parallax_speed_x=($item["plus_mouse_parallax_speed_x"]["size"]!='') ? $item["plus_mouse_parallax_speed_x"]["size"] : 30;
								$parallax_speed_y=($item["plus_mouse_parallax_speed_y"]["size"]!='') ? $item["plus_mouse_parallax_speed_y"]["size"] : 30;
							...
							if(!empty($item['typo_text'])){
								$loop_typo_text .= '<span class="plus-adv-text-typo elementor-repeater-item-' . esc_attr($item['_id']) . ' '.esc_attr($typo_underline_style).' '.esc_attr($animated_class).' ' . esc_attr($parallax_scroll) . ' '.esc_attr($parallax_move).'" ' . $magic_attr . ' '.$move_parallax_attr.' '.$animation_attr.'><'.$loop_tag.' '.$this->get_render_attribute_string( "loop_typo_link".$i ).' class="listing-typo-text '.esc_attr($data_class).' " '.$marquee_attr.' style="'.$mix_blend_style.'">';
								$loop_typo_text .= htmlspecialchars_decode($item['typo_text']);
								$loop_typo_text .= '</'.$loop_tag.'></span>';
							}
							if(!empty($magic_class) || !empty($move_parallax)){
								$loop_typo_text .= '</span>';
							}
					...
					if(!empty($advanced_typography_text)){
						...
						if($settings['marquee_switch']=='yes'){
							$marquee_attr='';
							$marquee_attr .= (!empty($settings['marquee_direction'])) ? ' direction="'.$settings['marquee_direction'].'"' : '';
							$marquee_attr .= (!empty($settings['marquee_behavior'])) ? ' behavior="'.$settings['marquee_behavior'].'"' : '';						
							$marquee_attr .= (!empty($settings['marquee_loop'])) ? ' loop="'.$settings['marquee_loop'].'"' : '';
							$marquee_attr .= (!empty($settings['marquee_scrollamount'])) ? ' scrollamount="'.$settings['marquee_scrollamount'].'"' : '';
							$marquee_attr .= (!empty($settings['marquee_scrolldelay'])) ? ' scrolldelay="'.$settings['marquee_scrolldelay'].'"' : '';
							...
							if (!empty($settings['text_link']['url'])){
								$adv_typ_text = '<a id="'.esc_attr($uidat).'" '.$this->get_render_attribute_string( "typo_link_a" ).' class="text-content-block '.esc_attr($circular_text_switch).' '.esc_attr($stroke_switch).' '.esc_attr($background_based_text_switch).' '.esc_attr($img_gif_ovly_txtimg_switch).' '.esc_attr($text_continuous_animation).' '.esc_attr($animated_class).'" '.$data_attr.' style="'.$text_write_mode.$mix_blend_attr.' " '.$circular_attr.' '.$animation_attr.'>'.htmlspecialchars_decode($loop_text).'</a>';						
							}else{
								$adv_typ_text = '<div id="'.esc_attr($uidat).'" class="text-content-block  '.esc_attr($circular_text_switch).' '.esc_attr($stroke_switch).' '.esc_attr($background_based_text_switch).' '.esc_attr($img_gif_ovly_txtimg_switch).' '.esc_attr($text_continuous_animation).' '.esc_attr($animated_class).'" '.$data_attr.' style="'.$text_write_mode.$mix_blend_attr.'" '.$circular_attr.' '.$animation_attr.'>'.htmlspecialchars_decode($loop_text).'</div>';
							}						
						}
					}
				}
			...
			$advanced_typography ='<div class="pt-plus-adv-typo-wrapper">';
			if(!empty($settings["typography_listing"]) && $settings["typography_listing"]=='listing'){
