/*Navigation Menu*/(function ($) {
		...
		if(inner_width <= 991 && $('.plus-mobile-menu-content').length){
			...
			if (!container.is(e.target) && container.has(e.target).length === 0){
				...
				if (!$menu.is(e.target) && $menu.has(e.target).length === 0){
					...
					$(".plus-mobile-menu-content").slideUp(400);
					$(".plus-mobile-menu-content").removeClass('in');
					$(".mobile-plus-toggle-menu").addClass("plus-collapsed");
				}
			}
			});
		}
	...
	$(window).on("load resize",function(e){
		...
		if($('.plus-mobile-menu-content:not(.nav-cust-width)').length){
			var offeset=$(".plus-mobile-menu-content:not(.nav-cust-width)").closest(".plus-navigation-wrap");
			var window_width = $(window).width();
			var menu_content=$(".plus-mobile-menu-content:not(.nav-cust-width)");
	...
	var WidgetHeaderNavigation = function($scope, $) {
		...
		if($plus_navigation.length > 0){
			...
			if (nav_sticky!=undefined && nav_sticky=='yes' && nav_sticky_id!='') {
                var sec_id = $plus_navigation.closest(".elementor-element.elementor-section").addClass("plus-nav-sticky-sec").data("id");
