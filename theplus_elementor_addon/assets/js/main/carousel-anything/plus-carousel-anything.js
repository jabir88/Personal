			...
			if($target.length){
				...
				$('.'+uid+' > .post-inner-loop').on('beforeChange', function(e, slick, currentSlide, nextSlide) {
					...
					if($('.'+uid).data("connection")!='' && $('.'+uid).data("connection")!=undefined){
						...
						if(!$("#"+connection).find('.info-box-inner[data-index="'+parseInt(nextSlide)+'"]').hasClass("tp-info-active")){
							var con__event = $("#"+connection).data("eventtype");					
							if(con__event=='con_pro_click'){							
								$("#"+connection).find('.info-box-inner[data-index="'+parseInt(nextSlide)+'"]').trigger("click");
							}else if(con__event=='con_pro_hover'){
								$("#"+connection).find('.info-box-inner[data-index="'+parseInt(nextSlide)+'"]').trigger("hover");
							}
