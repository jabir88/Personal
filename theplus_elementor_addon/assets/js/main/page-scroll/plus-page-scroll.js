		 ...
		 build: function() {
			...
			b.fullpage(a.extend(!0, {}, this.options, {
				...
				afterLoad: function(origin, destination, direction){
					...
					if(a("#"+uid).find(".fp-section .elementor-widget[data-settings]").length > 0){
						a(tp_anim_cls).find(".elementor-widget[data-settings].animated").each(function() {
							var t=a(this), b = a(this).data("settings");
							if(b._animation){
								t.addClass("elementor-invisible").removeClass(b._animation +" animated");
							}
						});
						a(tp_anim).find(".elementor-widget[data-settings]:not(.animated)").each(function() {
							var t=a(this), b = a(this).data("settings");
			...
			if(show_paginate=='on'){			
				...
				for(var i=0;i<slide_length;i++){
					if(i==0){
						 content_length+='<span class="slide-nav '+paginate_style+' active animated" data-slide="'+parseInt(i)+'">'+(i < 10 ? '0'+parseInt(i+1) : ''+parseInt(i+1))+'</span>';
					}else{
					content_length+='<span class="slide-nav '+paginate_style+' " data-slide="'+i+'">'+(i < 10 ? '0'+parseInt(i+1) : ''+parseInt(i+1))+'</span>';
					}
				}
	...
	var WidgetPageScrollHandler = function($scope, $) {		
		...
		if($scope.find('.tp-page-scroll-wrapper.tp_multi_scroll').length>0){
			...
			function multiScrollNormal() {
				...
				$(multi_scroll_elem).parents(".elementor-top-section").removeClass("elementor-section-height-full");			  
			}
