(function ($) {
	...
	$(document).ready(function(){
		if ($(".theplus-settings_page_post_type_options").length) {
		...
		if($(".cmb2-id-plus-smart-performance").length > 0 || $(".theplus_form_content #theplus_options").length > 0){
			...
			if($(".cmb2-id-plus-smart-performance").length > 0){
				$(".cmb2-id-plus-smart-performance > .cmb-td").append('<a href="#" id="'+ids+'" class="tp-smart-cache-btn">Purge All Caches</a><div class="smart-performace-desc-btn">Use above button to delete all cache our plugin have generated. It will start creating cache once some one start visiting your website.</div>');
				smart_action = "smart_perf_clear_cache";
			}
			if($(".theplus_form_content #theplus_options").length > 0){
				$(".theplus_form_content #theplus_options").append('<a href="#" id="'+ids+'" class="tp-smart-cache-btn">Remove Cache After Saving<div class="smart-performace-desc-tooltip">Remove backend cache after enabling/disabling widgets for performance.</div></a>');
				...
				if (confirmation) {
					...
					$.ajax({
						...
						success: function(response) {
							if($(".cmb2-id-plus-smart-performance").length > 0){
								setTimeout(function() {
									$this.html("Purge All Caches");
								}, 100);
							}
							if($(".theplus_form_content #theplus_options").length > 0){
								setTimeout(function() {
									$this.html('Remove Cache After Saving<div class="smart-performace-desc-tooltip">Remove backend cache after enabling/disabling widgets for performance.</div>');
								}, 100);
							}
						},
						error: function() {
